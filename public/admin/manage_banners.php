...
<body class="bg-[#111A2C] text-white flex h-screen overflow-hidden" x-data="{ sidebarOpen: false, modalOpen: false, editData: {} }">
                    ...
                    <div class="bg-[#1E2F4D] rounded-2xl shadow-sm border border-white/10 overflow-hidden group">
                        <div class="flex flex-col md:flex-row">
                            <div class="md:w-1/3 relative">
                                <div class="banner-preview bg-gray-900" style="background-image: url('<?= strpos($banner['gambar'], 'http') === 0 ? $banner['gambar'] : '../' . $banner['gambar'] ?>')"></div>
                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <button @click="editData = <?= htmlspecialchars(json_encode($banner)) ?>; modalOpen = true" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-full font-bold shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all">
                                        Edit Banner
                                    </button>
                                </div>
                            </div>
                            <div class="p-6 md:w-2/3 flex flex-col justify-center">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <span class="text-xs font-bold uppercase tracking-widest text-blue-400 mb-1 block">Halaman <?= ucfirst($banner['halaman']) ?></span>
                                        <h3 class="text-2xl font-bold text-white"><?= htmlspecialchars($banner['judul']) ?></h3>
                                    </div>
                                    <button @click="editData = <?= htmlspecialchars(json_encode($banner)) ?>; modalOpen = true" class="text-blue-400 hover:text-blue-300 transition">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" style="display: none;">
        <div @click.away="modalOpen = false" class="bg-[#1E2F4D] rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all">
            <div class="p-6 border-b border-white/10 flex justify-between items-center bg-[#1E2F4D]">
                <h3 class="text-xl font-bold text-white">Edit Banner: <span class="text-blue-400" x-text="editData.halaman?.toUpperCase()"></span></h3>
            ...
            <form action="manage_banners.php" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" name="judul" :value="editData.judul" required class="w-full px-4 py-3 rounded-xl bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-200 mb-2">Sub-judul (Deskripsi Singkat)</label>
                        <textarea name="subjudul" rows="3" x-text="editData.subjudul" required class="w-full px-4 py-3 rounded-xl bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-200 mb-2">Ganti Gambar Banner</label>
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-white/10 border-dashed rounded-2xl hover:border-blue-500 transition-colors bg-[#1E2F4D] group cursor-pointer relative">
                            <div class="space-y-1 text-center">
                                <svg class="mx-auto h-12 w-12 text-gray-400 group-hover:text-blue-400 transition-colors" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                    <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                                <div class="flex text-sm text-gray-300 justify-center">
                                    <span class="relative cursor-pointer bg-[#1E2F4D] rounded-md font-medium text-blue-400 hover:text-blue-300 focus-within:outline-none">Unggah file</span>
                ...
                <div class="pt-6 flex gap-4">
                    ...
                    <button type="submit" class="flex-1 px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
