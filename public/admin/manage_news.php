    ...
    <div x-show="showImageModal" 
        ...
        <div @click.away="showImageModal = false" 
            ...
            <div class="p-4 bg-[#1E2F4D] border-t border-white/10 flex justify-between items-center">
                ...
                <button type="button" @click="insertImageFromUrl(imageUrlInput, imageCaptionInput)" class="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-500/20">Konfirmasi</button>
            </div>
        </div>
    </div>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden w-full">
        <header class="h-20 bg-[#1E2F4D] shadow-sm flex items-center justify-between px-4 lg:px-8 z-10">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-xl lg:text-2xl font-bold text-white"><?= $edit_data ? 'Edit Berita' : 'Tambah Berita Baru' ?></h2>
            </div>
            <div class="flex items-center gap-3 lg:gap-4">
                <?php if($edit_data): ?>
                    <a href="manage_news.php" class="bg-white/5 hover:bg-white/10 px-3 py-1.5 lg:px-4 lg:py-2 rounded-lg text-[10px] lg:text-sm font-bold text-gray-200 transition border border-white/10">Batal</a>
                <?php endif; ?>
        ...
        <div class="flex-1 p-8 overflow-y-auto grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div class="xl:col-span-1 bg-[#1E2F4D] p-6 rounded-2xl shadow-sm border border-white/20 h-fit">
                ...
                <form action="manage_news.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-bold mb-1 text-gray-200">Judul Berita</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold mb-1 text-gray-200">Kategori</label>
                            <input type="text" name="kategori" value="<?= htmlspecialchars($edit_data['kategori'] ?? '') ?>" class="w-full p-2.5 rounded-lg bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Contoh: Kegiatan" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-1 text-gray-200">Warna Kategori</label>
                            <select name="kategori_color" class="w-full p-2.5 rounded-lg bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold mb-1 text-gray-200">Penulis</label>
                            <input type="text" name="penulis" value="<?= htmlspecialchars($edit_data['penulis'] ?? 'Admin') ?>" class="w-full p-2.5 rounded-lg bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-1 text-gray-200">Tanggal Posting</label>
                            <input type="date" name="tanggal_posting" value="<?= $edit_data['tanggal_posting'] ?? date('Y-m-d') ?>" class="w-full p-2.5 rounded-lg bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-1 text-gray-200">Gambar Berita</label>
                        <div class="space-y-3">
                            ...
                            <input type="file" name="gambar_file" accept="image/*" class="w-full p-2.5 rounded-lg bg-[#1B2945] border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-1 text-gray-200">Ringkasan (Pendek)</label>
                    ...
                    <div>
                        <label class="block text-sm font-bold mb-1 text-gray-200">Isi Berita (HTML Based)</label>
                        ...
                        <textarea id="isi-editor" name="isi" class="w-full p-3 rounded-lg bg-[#1B2945] border border-white/10 text-white font-mono text-sm leading-6 focus:outline-none focus:ring-2 focus:ring-blue-500" rows="10" required><?= htmlspecialchars($edit_data['isi'] ?? '') ?></textarea>
                        <input type="file" id="content-image-upload" class="hidden" accept="image/*">
                        <p class="text-xs text-gray-400 mt-2">Gunakan tombol di atas untuk menyisipkan tag HTML seperti <strong>&lt;strong&gt;</strong>, <strong>&lt;em&gt;</strong>, bullet list, dan list bernomor agar lebih mudah ditulis.</p>
                    </div>
            ...
            <div class="xl:col-span-2 space-y-8">
                ...
                <div class="bg-[#1E2F4D] rounded-2xl shadow-sm border border-white/20 overflow-hidden">
                    ...
                    <div class="p-8 max-w-2xl mx-auto">
                        ...
                        <h1 id="preview-title" class="text-3xl font-bold text-white mb-4 leading-tight">Judul Berita Akan Muncul Di Sini</h1>
                        <div class="flex items-center gap-3 mb-6 text-sm text-gray-400 pb-6 border-b border-white/10">
                            <span id="preview-author" class="font-semibold text-white">Penulis</span>
                            <span>•</span>
                        ...
                        <div id="preview-content" class="prose prose-blue max-w-none text-gray-300 leading-relaxed">
                ...
                <!-- List -->
                <div class="bg-[#1E2F4D] rounded-2xl shadow-sm border border-white/20 overflow-hidden">
                <div class="p-4 bg-[#1E2F4D] border-b border-white/10 font-bold">Daftar Berita</div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-white/10">
                            ...
                            <tr class="hover:bg-white/5 transition-colors">
                                ...
                                <td class="p-4 font-medium text-white"><?= htmlspecialchars($b['judul']) ?></td>
                                <td class="p-4 text-xs text-gray-300"><?= htmlspecialchars($b['kategori']) ?></td>
                                <td class="p-4 space-x-2">
                                    <a href="../detail-berita.php?slug=<?= $b['slug'] ?>" target="_blank" class="text-green-400 font-bold hover:text-green-300 hover:underline">Lihat</a>
                                    <a href="?edit=1&id=<?= $b['id'] ?>" class="text-blue-400 font-bold hover:text-blue-300 hover:underline">Edit</a>
                                    <a href="?delete=1&id=<?= $b['id'] ?>" class="text-red-400 font-bold hover:text-red-300 hover:underline" onclick="return confirm('Hapus berita ini?')">Hapus</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
